...
import { buildPickupWhatsAppMessage } from "@/lib/whatsapp-order-message";
...
export function PublicOrderPage() {
  ...
  const selectedItems = useMemo(
    () =>
      products
        .map((product) => ({
          product,
          quantity: formState.quantities[product.id] ?? 0,
        }))
        .filter((item) => item.quantity > 0),
    [formState.quantities, products],
  );
  ...
  const pickupWhatsAppUrl = useMemo(() => {
    ...
    return buildPublicWhatsAppUrl(message);
  }, [formState.customerName, formState.desiredDate, selectedItems]);

  function updateField<Field extends keyof ReturnType<typeof createInitialFormState>>(
    field: Field,
    value: ReturnType<typeof createInitialFormState>[Field],
  ) {
    setFormState((current) => ({
      ...current,
      [field]: value,
      ...
      const response = await fetch("/api/pedidos", {
        ...
        body: JSON.stringify({
          ...
          ...(formState.deliveryMethod === "DELIVERY"
            ...
            : {}),
          ...(formState.deliveryMethod === "DELIVERY" && trimmedAddressComplement
    ...
    return (
      <main className="min-h-screen bg-background">
        <PageContainer className="flex min-h-[calc(100vh-220px)] items-center justify-center py-10">
          <Card className="w-full max-w-xl space-y-5 p-6 text-center sm:p-8">
            <PageTitle
              eyebrow="Pedido recebido"
              title="Pedido enviado com sucesso!"
              subtitle="Ja vamos preparar tudo e te chamar com as proximas atualizacoes."
            />

            <p className="text-sm text-text-muted">
              Voce sera redirecionado automaticamente para o inicio em alguns segundos.
            </p>

            <div className="flex justify-center">
              <Link
                className="inline-flex min-h-12 items-center justify-center rounded-[var(--radius-control)] border border-border-strong bg-surface px-5 py-3 text-sm font-semibold text-foreground transition hover:bg-surface-muted"
                href="/"
              >
      ...
      <PageContainer className="space-y-6 pb-8">
        <Card className="space-y-4 p-5 sm:p-6">
          <PageTitle
            ...
            title="Monte seu pedido com rapidez e clareza"
            subtitle="Escolha os produtos, informe seus dados e finalize tudo em poucos passos."
          />

          <div className="flex flex-col gap-3 sm:flex-row">
            <Link
              className="inline-flex min-h-12 items-center justify-center rounded-[var(--radius-control)] border border-transparent bg-accent px-5 py-3 text-sm font-semibold text-text-contrast transition hover:bg-accent-strong"
              href="/"
            >
              Voltar para o inicio
            </Link>
                ...
                {formState.deliveryMethod === "DELIVERY" ? (
                  ...
                  <div className="rounded-[var(--radius-control)] border border-border-strong bg-background/25 p-4">
                    ...
                    <p className="mt-2 text-sm text-text-muted">
                      Vamos preparar seu pedido para retirada. Se quiser, voce ja pode
                      falar com a Giu pelo WhatsApp.
                    </p>
                    <Button
                      className="mt-4"
              ...
              formState.addressComplement ? (
                <div className="rounded-[var(--radius-control)] border border-border-soft bg-background/25 p-4">
                  ...
                  <div className="mt-2 space-y-2 text-sm text-text-muted">
                    <p>
                      Como receber:{" "}
                      {formatDeliveryMethodLabel(formState.deliveryMethod)}
                    </p>
                    {formState.desiredDate ? (
                      <p>
                        Para quando:{" "}
                        {formatOrderDesiredDate(formState.desiredDate)}
                      </p>
                    ) : null}
                    {formState.deliveryMethod === "DELIVERY" &&
                    formatOrderAddress({
                    ...
                    {formState.deliveryMethod === "DELIVERY" && formState.zipCode ? (
                      <p>CEP: {formatZipCode(formState.zipCode)}</p>
                    ) : null}
                    {formState.deliveryMethod === "PICKUP" ? (
